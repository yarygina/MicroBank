...
class dbCtrl():
    ...
    def getByUsername(self, username, json=False):
        try:
            ...
            self.logger.info('User (username=%s) is retrieved' % username)
            if json:
    ...
    def getById(self, userID, json=False):
        try:
            # A non existing username gives None
            u = User.query.filter_by(id=userID).first()
            self.logger.info('User (userID=%s) is retrieved' % userID)
            if json:
                return self.to_json(u)
            else:
                return u
    ...
    def isUserName(self, username):
        ...
        self.logger.info('Username (%s) in use' % username)
        return not (u is None)

    def isUserID(self, userID):
        u = self.getById(userID)
        self.logger.info('UserID (%s) in use' % userID)
        return not (u is None)

    def createUser(self, username, pwd):
        res = 1
        try:
            if not self.isUserName(username):
